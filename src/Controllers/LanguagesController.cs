...
namespace Bastille.Id.Server.Controllers.Api
{
    ...
    /// <summary>
    /// This controller class handles all language related calls.
    /// </summary>
    [Route("[controller]")]
    public class LanguagesController : ApiControllerBase
    {
        ...
        public LanguagesController(ApplicationContext<ApplicationSettings> appContext,
            UserManager<User> userManager, IAdvancedDistributedCache distributedCache, IMessageSender emailSender, ILogger<LanguagesController> logger)
            : base(appContext, userManager, distributedCache, emailSender, logger)
        {
        }

        /// <summary>
        /// Gets all available active languages
        /// </summary>
        ...
        {
            var results = await this.AppContext.DataContext.Languages
                .AsNoTracking()
                .Where(l => l.Active)
                .OrderBy(l => l.Name)
